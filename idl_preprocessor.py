...
def preprocess_directory(
    ...
    for idl_file in input_dir.glob("*.idl"):
        ...
        pp_proc = subprocess.Popen(
            shlex.split(pp_cmd), stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def main():
    ...
    parser.add_argument(
        "--pp",
        help='Path to IDL preprocessor e.g. "path/to/cl.exe" for MSVC',
        required=True,
    )
    parser.add_argument(
        "--args",
        help='Preprocessor arguments e.g. "/EP" for cl.exe',
        required=False,
        default="",
    )
    parser.add_argument(
