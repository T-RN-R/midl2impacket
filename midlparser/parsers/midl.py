...
class MidlParser(MidlBaseParser):
    ...
    def _midl_pragma(self, _):
        ...
        SkipClosureParser(self.tokens, self.tokenizer, "(", ")").parse(
            next(self.tokens)
        )
    ...
    def _dispinterface(self, token: Token):
        dispinterface = MidlDispInterfaceParser(self.tokens, self.tokenizer).parse(
            token
        )
    ...
    def sqbracket(self, token: Token):
        ...
        if token.data == "[" and self.state in [
            MidlState.DEFAULT,
            MidlState.DEF_COMPLETE,
        ]:
            self.cur_def_attrs.update(
