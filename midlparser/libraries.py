...
class MidlLibraryParser(MidlBaseParser):
    ...
    def __init__(self, token_generator, tokenizer):
        ...
        super().__init__(token_generator=token_generator, end_state=LibraryState.END, tokenizer=tokenizer)
    ...
    def default_type_handler(self, token):
    ...
    def brace(self, token):
        if token.data == "{" and self.state == LibraryState.BODY:
            self.state = LibraryState.MEMBER_TYPE_OR_ATTR
        elif token.data == "}" and self.state == LibraryState.MEMBER_TYPE_OR_ATTR and not self.cur_member_attrs:
