...
class CPREP_DISKID_ENUM(NdrEnum):
    MAP = ((0 , 'CprepIdSignature'),(1 , 'CprepIdGuid'),(4000 , 'CprepIdNumber'),(5000 , 'CprepIdUnknown'),)        

class CPREP_DISKID(NdrStructure):
    MEMBERS = [(CPREP_DISKID_ENUM, "DiskIdType"),(U0, "u0"),]

    
PCPREP_DISKID = CPREP_DISKID

class DISKSTACKTYPE(NdrEnum):
    MAP = ((0 , 'DiskStackScsiPort'),(1 , 'DiskStackStorPort'),(2 , 'DiskStackFullPort'),)        

class CPREP_SCSI_ADDRESS(NdrStructure):
    MEMBERS = [(UNSIGNED_LONG, "Length"),(UNSIGNED_CHAR, "PortNumber"),(UNSIGNED_CHAR, "PathId"),(UNSIGNED_CHAR, "TargetId"),(UNSIGNED_CHAR, "Lun"),]

    
PCPREP_SCSI_ADDRESS = CPREP_SCSI_ADDRESS

class DISK_PROPS(NdrStructure):
    MEMBERS = [(UNSIGNED_LONG, "DiskNumber"),(CPREP_DISKID, "DiskId"),(UNSIGNED_LONG, "DiskBusType"),(DISKSTACKTYPE, "StackType"),(CPREP_SCSI_ADDRESS, "ScsiAddress"),(LONG, "DiskIsClusterable"),(WCHAR_T, "AdapterDesc"),(UNSIGNED_LONG, "NumPaths"),(UNSIGNED_LONG, "Flags"),]

    
PDISK_PROPS = DISK_PROPS

class DISK_PROPS_EX(NdrStructure):
    MEMBERS = [(ULONG, "DiskNumber"),(CPREP_DISKID, "DiskId"),(ULONG, "DiskBusType"),(DISKSTACKTYPE, "StackType"),(CPREP_SCSI_ADDRESS, "ScsiAddress"),(BOOL, "DiskIsClusterable"),(WCHAR_T, "AdapterDesc"),(LPWSTR, "pwszFriendlyName"),(UNSIGNED_LONG, "NumPaths"),(UNSIGNED_LONG, "Flags"),(UNSIGNED_LONG, "ExtendedFlags"),(LPWSTR, "pwszPoolName"),(LPWSTR, "pwszPage83Id"),(LPWSTR, "pwszSerialNumber"),(GUID, "guidPoolId"),]

    
PDISK_PROPS_EX = DISK_PROPS_EX

class CLUSTER_NETWORK_PROFILE(NdrEnum):
    MAP = ((0 , 'ClusterNetworkProfilePublic'),(1 , 'ClusterNetworkProfilePrivate'),(2 , 'ClusterNetworkProfileDomainAuthenticated'),)        

class NODE_ROUTE_INFO(NdrStructure):
    MEMBERS = [(BSTR, "remoteVirtualIP"),(SAFEARRAY, "localUnicastIPs"),(SAFEARRAY, "remoteUnicastIPs"),(SAFEARRAY, "indices"),]

    

class ADD_ROUTES_REQUEST(NdrStructure):
    MEMBERS = [(BSTR, "localVirtualIP"),(SAFEARRAY, "nodeRouteInfos"),]

    

class ROUTE_STATUS(NdrEnum):
    MAP = ((0 , 'DOWN'),(1 , 'UP'),(2 , 'UP_DOWN'),)        

class ROUTE_LOSS_AND_STATE(NdrStructure):
    MEMBERS = [(ULONG, "packetLoss"),(ROUTE_STATUS, "status"),]

    

class ADD_ROUTES_REPLY(NdrStructure):
    MEMBERS = [(SAFEARRAY, "indices"),(SAFEARRAY, "replies"),(BOOLEAN, "routeUnavailable"),]

    

class DISKMEDIATYPE(NdrEnum):
    MAP = ((0 , 'DiskMediaTypeUnknown'),(1 , 'DiskMediaTypeHDD'),(2 , 'DiskMediaTypeSSD'),(3 , 'DiskMediaTypeSCM'),)        

class CLUSTERLOGEXFLAG(NdrEnum):
    MAP = ((0 , 'ClusterLogFlagNone'),(1 , 'ClusterLogFlagLocalTime'),(2 , 'ClusterLogFlagSkipClusterState'),)        

class CLUSTER_CERTTYPE(NdrEnum):
    MAP = ((0 , 'Cluster_SChannel'),(1 , 'ClusterSet_SChannel'),(2 , 'Cluster_PKU2U'),(3 , 'ClusterSet_PKU2U'),)        

class CLUSTER_CERT(NdrStructure):
    MEMBERS = [(ULONG, "CbCertData"),(ULONG, "CbKeyData"),(BYTE, "CertData"),(BYTE, "KeyData"),(WCHAR, "ClusterSecret"),]

    
Interface("12108A88-6858-4467-B92F-E6CF4568DFB6", "1.0",[Method("CprepDiskRawRead",
In((CPREP_DISKID,'DiskId')),
In((UNSIGNED_LONG,'ulSector')),
In((UNSIGNED_LONG,'cbData')),
Out((PBYTE,'pbData')),
Out((PUNSIGNED_LONG,'pcbDataRead')),
Out((PUNSIGNED_LONG,'ulLatency')),
),Method("CprepDiskRawWrite",
In((CPREP_DISKID,'DiskId')),
In((UNSIGNED_LONG,'ulSector')),
In((UNSIGNED_LONG,'cbData')),
In((PBYTE,'pbData')),
Out((PUNSIGNED_LONG,'pcbDataWritten')),
Out((PUNSIGNED_LONG,'ulLatency')),
),Method("CprepPrepareNode",
Out((PUNSIGNED_LONG,'pulMajorVersion')),
Out((PUNSIGNED_LONG,'pulMinorVersion')),
Out((PUNSIGNED_LONG,'pdwCPrepVersion')),
),Method("CprepPrepareNodePhase2",
In((UNSIGNED_LONG,'Flags')),
Out((PUNSIGNED_LONG,'pulNumDisks')),
),Method("CprepDiskGetProps",
In((CPREP_DISKID,'DiskId')),
Out((PDISK_PROPS,'DiskProps')),
...
In((CPREP_DISKID,'DiskId')),
Out((PUNSIGNED_LONG,'MaxPartitionNumber')),
...
In((CPREP_DISKID,'DiskId')),
In((UNSIGNED_LONG,'ulPartition')),
In((PWCHAR_T,'FileName')),
In((UNSIGNED_LONG,'cbDataIn')),
In((PBYTE,'DataIn')),
...
In((UNSIGNED_LONG,'cbData')),
Out((PBYTE,'pbData')),
Out((PUNSIGNED_LONG,'pcbDataOut')),
Out((PUNSIGNED_LONG,'pcbNeeded')),
...
In((CPREP_DISKID,'DiskId')),
Out((PUNSIGNED_LONG,'SectorX')),
Out((PUNSIGNED_LONG,'SectorY')),
),Method("CprepDiskIsPRPresent",
In((CPREP_DISKID,'DiskId')),
Out((PUNSIGNED_LONG,'Present')),
...
In((CPREP_DISKID,'DiskId')),
In((UNSIGNED_LONG,'Partition')),
Out((WCHAR_T,'FsName')),
),Method("CprepDiskIsReadable",
In((CPREP_DISKID,'DiskId')),
),Method("CprepDiskGetDsms",
In((UNSIGNED_LONG,'Size')),
Out((PUNSIGNED_LONG,'pReserved')),
Out((PBYTE,'RegisteredDsms')),
),])