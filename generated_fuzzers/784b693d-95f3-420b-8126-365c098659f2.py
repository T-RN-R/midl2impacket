...
class CERTTRANSBLOB(NdrStructure):
    MEMBERS = [(UNSIGNED_LONG, "cb"),(PBYTE, "pb"),]

    
Interface("784b693d-95f3-420b-8126-365c098659f2", "1.0",[Method("GetOCSPProperty",
In((BSTR,'bstrEntryName')),
Out((PVARIANT,'pEntryValue')),
),Method("SetOCSPProperty",
In((BSTR,'bstrEntryName')),
In((PVARIANT,'pEntryValue')),
...
Out((PCERTTRANSBLOB,'pctbSD')),
),Method("SetSecurity",
In((PCERTTRANSBLOB,'pctbSD')),
),Method("GetSigningCertificates",
In((PVARIANT,'pCAVar')),
Out((PVARIANT,'pSigningCertificates')),
),Method("GetHashAlgorithms",
In((BSTR,'bstrCAId')),
Out((PVARIANT,'pHashAlgorithms')),
),Method("GetMyRoles",
Out((PLONG,'pdwRoles')),
),Method("Ping",
),])