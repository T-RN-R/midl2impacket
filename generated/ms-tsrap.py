...
#IManageTelnetSessions Definition

#################################################################################

MSRPC_UUID_IMANAGETELNETSESSIONS = uuidtup_to_bin(('034634FD-BA3F-11D1-856A-00A0C944138C','0.0'))


class GetTelnetSessions(NDRCALL):
    opnum = 0
    structure = (

    )

class GetTelnetSessionsResponse(NDRCALL):
    structure = (
		('PSZSESSIONDATA', BSTR),
    )
        

class TerminateSession(NDRCALL):
    opnum = 1
    structure = (
		('DWUNIQUEID', DWORD),
    )

class TerminateSessionResponse(NDRCALL):
    structure = (

    )
        

class SendMsgToASession(NDRCALL):
    opnum = 2
    structure = (
		('DWUNIQUEID', DWORD),
		('SZMSG', BSTR),
...
class SendMsgToASessionResponse(NDRCALL):
    structure = (

    )
        
OPNUMS = {
0 : (GetTelnetSessions,GetTelnetSessionsResponse),
1 : (TerminateSession,TerminateSessionResponse),
2 : (SendMsgToASession,SendMsgToASessionResponse),
}
