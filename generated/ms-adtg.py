...
#IDataFactory Definition

#################################################################################

MSRPC_UUID_IDATAFACTORY = uuidtup_to_bin(('0EAC4842-8763-11cf-A743-00AA00A3F00D','0.0'))
...
class SubmitChanges(NDRCALL):
    opnum = 1
    structure = (
		('BSTRCONNECTION', BSTR),
		('PRECORDSET', IDISPATCH),
    )

class SubmitChangesResponse(NDRCALL):
    structure = (

    )
        

class ConvertToString(NDRCALL):
    opnum = 2
    structure = (
		('PUNKOBJECT', IUNKNOWN),
    )

class ConvertToStringResponse(NDRCALL):
    structure = (
		('PBSTRINLINE', BSTR),
    )
        

class CreateRecordSet(NDRCALL):
    opnum = 3
    structure = (
		('VARCOLUMNINFOS', VARIANT),
    )

class CreateRecordSetResponse(NDRCALL):
    structure = (
...
1 : (SubmitChanges,SubmitChangesResponse),
2 : (ConvertToString,ConvertToStringResponse),
3 : (CreateRecordSet,CreateRecordSetResponse),
}

#################################################################################

#IDataFactory2 Definition

#################################################################################

MSRPC_UUID_IDATAFACTORY2 = uuidtup_to_bin(('070669EB-B52F-11d1-9270-00C04FBBBFB3','0.0'))


class Execute21(NDRCALL):
    opnum = 0
    structure = (
		...
		('PROPERTIES', VARIANT),
		('TABLEID', VARIANT),
		('LEXECUTEOPTIONS', LONG),
		('PPARAMETERS', VARIANT),
    )

class Execute21Response(NDRCALL):
    structure = (
		('PPARAMETERS', VARIANT),
		('PPRECORDSET', IDISPATCH),
    )
...
class Synchronize21(NDRCALL):
    opnum = 1
    structure = (
		('CONNECTIONSTRING', BSTR),
		('HANDLERSTRING', BSTR),
		('LSYNCHRONIZEOPTIONS', LONG),
		('PPRECORDSET', IDISPATCH),
		('PSTATUSARRAY', VARIANT),
    )

class Synchronize21Response(NDRCALL):
    structure = (
		('PPRECORDSET', IDISPATCH),
		('PSTATUSARRAY', VARIANT),
		('PRESULT', VARIANT),
...
OPNUMS = {
0 : (Execute21,Execute21Response),
1 : (Synchronize21,Synchronize21Response),
}

#################################################################################

#IDataFactory3 Definition

#################################################################################

MSRPC_UUID_IDATAFACTORY3 = uuidtup_to_bin(('4639DB2A-BFC5-11d2-9318-00C04FBBBFB3','0.0'))
...
class SynchronizeResponse(NDRCALL):
    structure = (
		('PPRECORDSET', IDISPATCH),
		('PSTATUSARRAY', VARIANT),
		('PINFORMATION', VARIANT),
...
OPNUMS = {
0 : (Execute,ExecuteResponse),
1 : (Synchronize,SynchronizeResponse),
}
