...
class SAFEARRAYUNION(NDRSTRUCT):
    structure = (
        ('sfType', UNSIGNED_LONG),('Anonymous8', ANONYMOUS8),
    )
...
class CERTTRANSDBATTRIBUTE(NDRSTRUCT):
    structure = (
        ('obwszName', ULONG),('obwszValue', ULONG),
    )


class CERTTRANSDBEXTENSION(NDRSTRUCT):
    structure = (
        ('obwszName', ULONG),('ExtFlags', LONG),('cbValue', DWORD),('obValue', ULONG),
    )


class CERTTRANSDBCOLUMN(NDRSTRUCT):
    structure = (
        ('Type', DWORD),('Index', DWORD),('cbMax', DWORD),('obwszName', ULONG),('obwszDisplayName', ULONG),
    )


class CERTTRANSDBRESULTCOLUMN(NDRSTRUCT):
    structure = (
        ('Type', DWORD),('Index', DWORD),('obValue', ULONG),('cbValue', DWORD),
    )


class CERTTRANSDBRESULTROW(NDRSTRUCT):
    structure = (
        ('rowid', DWORD),('ccol', DWORD),('cbrow', ULONG),
    )


class DATA_CERTVIEWRESTRICTION(NDRUniConformantArray):
    item = BYTE

class PTR_CERTVIEWRESTRICTION(NDRPOINTER):
    referent = (
        ('Data', DATA_CERTVIEWRESTRICTION),
    )

class CERTVIEWRESTRICTION(NDRSTRUCT):
    structure = (
	('ColumnIndex', DWORD),	('SeekOperator', LONG),	('SortOrder', LONG),	('pbValue', PTR_CERTVIEWRESTRICTION),
	('cbValue', DWORD),
    )
...
#ICertAdminD Definition

#################################################################################

MSRPC_UUID_ICERTADMIND = uuidtup_to_bin(('d99e6e71-fc88-110-b498-000903123','0.0'))


class SetExtension(NDRCALL):
    opnum = 0
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('DWREQUESTID', DWORD),
		('PWSZEXTENSIONNAME', WCHAR_T ),
		('DWTYPE', DWORD),
		('DWFLAGS', DWORD),
		('PCTBVALUE', CERTTRANSBLOB),
    )

class SetExtensionResponse(NDRCALL):
    structure = (

    )
        

class SetAttributes(NDRCALL):
    opnum = 1
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('DWREQUESTID', DWORD),
...
class SetAttributesResponse(NDRCALL):
    structure = (

    )
        

class ResubmitRequest(NDRCALL):
    opnum = 2
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('DWREQUESTID', DWORD),
...
class ResubmitRequestResponse(NDRCALL):
    structure = (
		('PDWDISPOSITION', DWORD),
    )
        

class DenyRequest(NDRCALL):
    opnum = 3
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('DWREQUESTID', DWORD),
...
class DenyRequestResponse(NDRCALL):
    structure = (

    )
        

class IsValidCertificate(NDRCALL):
    opnum = 4
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('PSERIALNUMBER', WCHAR_T ),
    )

class IsValidCertificateResponse(NDRCALL):
    structure = (
		('PREVOCATIONREASON', LONG),
		('PDISPOSITION', LONG),
    )
        

class PublishCRL(NDRCALL):
    opnum = 5
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('FILETIME', FILETIME),
...
class PublishCRLResponse(NDRCALL):
    structure = (

    )
        

class GetCRL(NDRCALL):
    opnum = 6
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
    )

class GetCRLResponse(NDRCALL):
    structure = (
		('PCTBCRL', CERTTRANSBLOB),
    )
        

class RevokeCertificate(NDRCALL):
    opnum = 7
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
...
class RevokeCertificateResponse(NDRCALL):
    structure = (

    )
        

class EnumViewColumn(NDRCALL):
    opnum = 8
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('ICOLUMN', DWORD),
...
class EnumViewColumnResponse(NDRCALL):
    structure = (
		('PCCOLUMN', DWORD),
...
class GetViewDefaultColumnSet(NDRCALL):
    opnum = 9
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('ICOLUMNSETDEFAULT', DWORD),
    )

class GetViewDefaultColumnSetResponse(NDRCALL):
    structure = (
		('PCCOLUMN', DWORD),
		('PCTBCOLUMNINFO', CERTTRANSBLOB),
    )
...
class EnumAttributesOrExtensions(NDRCALL):
    opnum = 10
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('ROWID', DWORD),
...
class EnumAttributesOrExtensionsResponse(NDRCALL):
    structure = (
		('PCELTFETCHED', DWORD),
...
class OpenView(NDRCALL):
    opnum = 11
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('CCVR', DWORD),
		('ACVR', CERTVIEWRESTRICTION ),
		('CCOLOUT', DWORD),
		('ACOLOUT', DWORD ),
		('IELT', DWORD),
		('CELT', DWORD),
...
class OpenViewResponse(NDRCALL):
    structure = (
		('PCELTFETCHED', DWORD),
...
class EnumViewResponse(NDRCALL):
    structure = (
		('PCELTFETCHED', DWORD),
		('PCTBRESULTROWS', CERTTRANSBLOB),
    )
...
class CloseView(NDRCALL):
    opnum = 13
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
    )

class CloseViewResponse(NDRCALL):
    structure = (

    )
        

class ServerControl(NDRCALL):
    opnum = 14
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
...
class ServerControlResponse(NDRCALL):
    structure = (
		('PCTBOUT', CERTTRANSBLOB),
    )
...
class GetServerState(NDRCALL):
    opnum = 16
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
    )

class GetServerStateResponse(NDRCALL):
    structure = (
		('PDWSTATE', DWORD),
    )
        

class BackupPrepare(NDRCALL):
    opnum = 17
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('GRBITJET', UNSIGNED_LONG),
		('DWBACKUPFLAGS', UNSIGNED_LONG),
		('PWSZBACKUPANNOTATION', WCHAR ),
		('DWCLIENTIDENTIFIER', DWORD),
    )

class BackupPrepareResponse(NDRCALL):
    structure = (

    )
        

class BackupEnd(NDRCALL):
    opnum = 18
    structure = (

    )

class BackupEndResponse(NDRCALL):
    structure = (

    )
        

class BackupGetAttachmentInformation(NDRCALL):
    opnum = 19
    structure = (

    )

class BackupGetAttachmentInformationResponse(NDRCALL):
    structure = (
		('PPWSZZDBFILES', WCHAR),
		('PCWCDBFILES', LONG),
    )
...
class BackupGetBackupLogs(NDRCALL):
    opnum = 20
    structure = (

    )

class BackupGetBackupLogsResponse(NDRCALL):
    structure = (
		('PPWSZZLOGFILES', WCHAR),
		('PCWCLOGFILES', LONG),
    )
        

class BackupOpenFile(NDRCALL):
    opnum = 21
    structure = (
		('PWSZPATH', WCHAR_T ),
    )

class BackupOpenFileResponse(NDRCALL):
    structure = (
		('PLILENGTH', UNSIGNED_HYPER),
    )
        

class BackupReadFile(NDRCALL):
    opnum = 22
    structure = (
		('CBBUFFER', LONG),
    )

class BackupReadFileResponse(NDRCALL):
    structure = (
		('PBBUFFER', BYTE),
		('PCBREAD', LONG),
    )
...
class BackupCloseFile(NDRCALL):
    opnum = 23
    structure = (

    )

class BackupCloseFileResponse(NDRCALL):
    structure = (

    )
        

class BackupTruncateLogs(NDRCALL):
    opnum = 24
    structure = (

    )

class BackupTruncateLogsResponse(NDRCALL):
    structure = (

    )
        

class ImportCertificate(NDRCALL):
    opnum = 25
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('PCTBCERTIFICATE', CERTTRANSBLOB),
		('DWFLAGS', LONG),
    )

class ImportCertificateResponse(NDRCALL):
    structure = (
		('PDWREQUESTID', LONG),
    )
        

class BackupGetDynamicFiles(NDRCALL):
    opnum = 26
    structure = (

    )

class BackupGetDynamicFilesResponse(NDRCALL):
    structure = (
		('PPWSZZFILES', WCHAR),
		('PCWCFILES', LONG),
    )
...
class RestoreGetDatabaseLocations(NDRCALL):
    opnum = 27
    structure = (

    )

class RestoreGetDatabaseLocationsResponse(NDRCALL):
    structure = (
		('PPWSZZDATABASELOCATIONS', WCHAR),
		('PCWCPATHS', LONG),
    )
        
OPNUMS = {
0 : (SetExtension,SetExtensionResponse),
1 : (SetAttributes,SetAttributesResponse),
2 : (ResubmitRequest,ResubmitRequestResponse),
3 : (DenyRequest,DenyRequestResponse),
4 : (IsValidCertificate,IsValidCertificateResponse),
5 : (PublishCRL,PublishCRLResponse),
6 : (GetCRL,GetCRLResponse),
7 : (RevokeCertificate,RevokeCertificateResponse),
8 : (EnumViewColumn,EnumViewColumnResponse),
9 : (GetViewDefaultColumnSet,GetViewDefaultColumnSetResponse),
10 : (EnumAttributesOrExtensions,EnumAttributesOrExtensionsResponse),
11 : (OpenView,OpenViewResponse),
12 : (EnumView,EnumViewResponse),
13 : (CloseView,CloseViewResponse),
14 : (ServerControl,ServerControlResponse),
15 : (Ping,PingResponse),
16 : (GetServerState,GetServerStateResponse),
17 : (BackupPrepare,BackupPrepareResponse),
18 : (BackupEnd,BackupEndResponse),
19 : (BackupGetAttachmentInformation,BackupGetAttachmentInformationResponse),
20 : (BackupGetBackupLogs,BackupGetBackupLogsResponse),
21 : (BackupOpenFile,BackupOpenFileResponse),
22 : (BackupReadFile,BackupReadFileResponse),
23 : (BackupCloseFile,BackupCloseFileResponse),
24 : (BackupTruncateLogs,BackupTruncateLogsResponse),
25 : (ImportCertificate,ImportCertificateResponse),
26 : (BackupGetDynamicFiles,BackupGetDynamicFilesResponse),
27 : (RestoreGetDatabaseLocations,RestoreGetDatabaseLocationsResponse),
}

#################################################################################

#ICertAdminD2 Definition

#################################################################################

MSRPC_UUID_ICERTADMIND2 = uuidtup_to_bin(('7fe0d935-dda6-443f-85d0-1cfb58fe41dd','0.0'))


class PublishCRLs(NDRCALL):
    opnum = 0
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('FILETIME', FILETIME),
...
class PublishCRLsResponse(NDRCALL):
    structure = (
...
class SetCAPropertyResponse(NDRCALL):
    structure = (
...
class EnumViewColumnTable(NDRCALL):
    opnum = 4
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('ITABLE', DWORD),
...
class EnumViewColumnTableResponse(NDRCALL):
    structure = (
		('PCCOLUMN', DWORD),
		('PCTBCOLUMNINFO', CERTTRANSBLOB),
    )
...
class GetCASecurity(NDRCALL):
    opnum = 5
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
    )

class GetCASecurityResponse(NDRCALL):
    structure = (
		('PCTBSD', CERTTRANSBLOB),
    )
        

class SetCASecurity(NDRCALL):
    opnum = 6
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('PCTBSD', CERTTRANSBLOB),
...
class SetCASecurityResponse(NDRCALL):
    structure = (
...
class GetArchivedKeyResponse(NDRCALL):
    structure = (
		('PCTBARCHIVEDKEY', CERTTRANSBLOB),
    )
        

class GetAuditFilter(NDRCALL):
    opnum = 9
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
    )

class GetAuditFilterResponse(NDRCALL):
    structure = (
		('PDWFILTER', DWORD),
    )
        

class SetAuditFilter(NDRCALL):
    opnum = 10
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('DWFILTER', DWORD),
...
class SetAuditFilterResponse(NDRCALL):
    structure = (

    )
        

class GetOfficerRights(NDRCALL):
    opnum = 11
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
    )

class GetOfficerRightsResponse(NDRCALL):
    structure = (
		('PFENABLED', BOOL),
		('PCTBSD', CERTTRANSBLOB),
    )
...
class SetOfficerRights(NDRCALL):
    opnum = 12
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('FENABLE', BOOL),
...
class SetOfficerRightsResponse(NDRCALL):
    structure = (

    )
        

class GetConfigEntry(NDRCALL):
    opnum = 13
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('PWSZNODEPATH', WCHAR_T ),
		('PWSZENTRY', WCHAR_T ),
    )

class GetConfigEntryResponse(NDRCALL):
    structure = (
		('PVARIANT', VARIANT),
    )
        

class SetConfigEntry(NDRCALL):
    opnum = 14
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('PWSZNODEPATH', WCHAR_T ),
...
class SetConfigEntryResponse(NDRCALL):
    structure = (

    )
        

class ImportKey(NDRCALL):
    opnum = 15
    structure = (
		('PWSZAUTHORITY', WCHAR_T ),
		('DWREQUESTID', DWORD),
		('PWSZCERTHASH', WCHAR_T ),
		('DWFLAGS', DWORD),
		('PCTBKEY', CERTTRANSBLOB),
    )

class ImportKeyResponse(NDRCALL):
    structure = (
...
class DeleteRowResponse(NDRCALL):
    structure = (
		('PCDELETED', LONG),
    )
        
OPNUMS = {
0 : (PublishCRLs,PublishCRLsResponse),
1 : (GetCAProperty,GetCAPropertyResponse),
2 : (SetCAProperty,SetCAPropertyResponse),
3 : (GetCAPropertyInfo,GetCAPropertyInfoResponse),
4 : (EnumViewColumnTable,EnumViewColumnTableResponse),
5 : (GetCASecurity,GetCASecurityResponse),
6 : (SetCASecurity,SetCASecurityResponse),
7 : (Ping2,Ping2Response),
8 : (GetArchivedKey,GetArchivedKeyResponse),
9 : (GetAuditFilter,GetAuditFilterResponse),
10 : (SetAuditFilter,SetAuditFilterResponse),
11 : (GetOfficerRights,GetOfficerRightsResponse),
12 : (SetOfficerRights,SetOfficerRightsResponse),
13 : (GetConfigEntry,GetConfigEntryResponse),
14 : (SetConfigEntry,SetConfigEntryResponse),
15 : (ImportKey,ImportKeyResponse),
16 : (GetMyRoles,GetMyRolesResponse),
17 : (DeleteRow,DeleteRowResponse),
}
