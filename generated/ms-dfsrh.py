...
dfsrHelperErrorNotLocalAdmin = 2147753985,
dfsrHelperErrorCreateVerifyServerControl = 2147753986,
dfsrHelperLdapErrorBase = 2147758080
...
class _ADATTRIBUTEDATA(NDRSTRUCT):
    structure = (
        ('operation', LONG),('attributeName', BSTR),('attributeValue', BSTR),('isString', VARIANT_BOOL),('length', LONG),
    )


class _VERSIONVECTORDATA(NDRSTRUCT):
    structure = (
        ('uncompressedSize', LONG),('backlogCount', LONG),('contentSetGuid', BSTR),('versionVector', VARIANT),
    )
...
MSRPC_UUID_IADPROXY = uuidtup_to_bin(('4BB8AB1D-9EF9-4100-8EB6-DD4B4E418B72','0.0'))


class CreateObject(NDRCALL):
    opnum = 0
    structure = (
		('DOMAINCONTROLLERNAME', BSTR),
		('DISTINGUISHEDNAME', BSTR),
		('ATTRIBUTES', SAFEARRAY ( _ADATTRIBUTEDATA )),
		('VERIFYNAMEDOMAINCONTROLLERNAME', BSTR),
    )

class CreateObjectResponse(NDRCALL):
    structure = (

    )
        

class DeleteObject(NDRCALL):
    opnum = 1
    structure = (
		('DOMAINCONTROLLERNAME', BSTR),
		('DISTINGUISHEDNAME', BSTR),
...
class DeleteObjectResponse(NDRCALL):
    structure = (

    )
        

class ModifyObject(NDRCALL):
    opnum = 2
    structure = (
		('DOMAINCONTROLLERNAME', BSTR),
		('DISTINGUISHEDNAME', BSTR),
...
class ModifyObjectResponse(NDRCALL):
    structure = (

    )
        
OPNUMS = {
0 : (CreateObject,CreateObjectResponse),
1 : (DeleteObject,DeleteObjectResponse),
2 : (ModifyObject,ModifyObjectResponse),
}

#################################################################################

#IADProxy2 Definition

#################################################################################

MSRPC_UUID_IADPROXY2 = uuidtup_to_bin(('C4B0C7D9-ABE0-4733-A1E1-9DEDF260C7A','0.0'))
...
#IServerHealthReport Definition

#################################################################################

MSRPC_UUID_ISERVERHEALTHREPORT = uuidtup_to_bin(('E65E8028-838-491-9F7-AAF6BD51A0CE','0.0'))


class GetReport(NDRCALL):
    opnum = 0
    structure = (
		('REPLICATIONGROUPGUID', GUID),
		('REFERENCEMEMBER', BSTR),
		('REFERENCEVERSIONVECTORS', SAFEARRAY ( _VERSIONVECTORDATA )),
		('FLAGS', LONG),
    )

class GetReportResponse(NDRCALL):
    structure = (
		('MEMBERVERSIONVECTORS', SAFEARRAY ( _VERSIONVECTORDATA )),
		('REPORTXML', BSTR),
    )
...
class GetCompressedReportResponse(NDRCALL):
    structure = (
		('MEMBERVERSIONVECTORS', SAFEARRAY ( _VERSIONVECTORDATA )),
		('REPORTCOMPRESSED', BSTR),
		('UNCOMPRESSEDREPORTSIZE', LONG),
    )
        

class GetRawReportEx(NDRCALL):
    opnum = 2
    structure = (
		('REPLICATIONGROUPGUID', GUID),
		('REPORTOPTIONS', SAFEARRAY ( VARIANT )),
    )

class GetRawReportExResponse(NDRCALL):
    structure = (
		('REPORT', SAFEARRAY ( VARIANT )),
    )
        

class GetReferenceVersionVectors(NDRCALL):
    opnum = 3
    structure = (
		('REPLICATIONGROUPGUID', GUID),
    )

class GetReferenceVersionVectorsResponse(NDRCALL):
    structure = (
		('REFERENCEVERSIONVECTORS', SAFEARRAY ( _VERSIONVECTORDATA )),
    )
...
class GetReferenceBacklogCounts(NDRCALL):
    opnum = 5
    structure = (
		('FLATMEMBERVERSIONVECTORS', SAFEARRAY ( _VERSIONVECTORDATA )),
    )

class GetReferenceBacklogCountsResponse(NDRCALL):
    structure = (
		('BACKLOGCOUNTS', SAFEARRAY ( LONG )),
    )
        
OPNUMS = {
0 : (GetReport,GetReportResponse),
1 : (GetCompressedReport,GetCompressedReportResponse),
2 : (GetRawReportEx,GetRawReportExResponse),
3 : (GetReferenceVersionVectors,GetReferenceVersionVectorsResponse),
4 : (Opnum7NotUsedOnWire,Opnum7NotUsedOnWireResponse),
5 : (GetReferenceBacklogCounts,GetReferenceBacklogCountsResponse),
}

#################################################################################

#IServerHealthReport2 Definition

#################################################################################

MSRPC_UUID_ISERVERHEALTHREPORT2 = uuidtup_to_bin(('20D15747-6C48-4254-A358-65039FD8C63C','0.0'))
