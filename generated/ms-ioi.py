...
class SAFEARRAYUNION(NDRSTRUCT):
    structure = (
        ('sfType', UNSIGNED_LONG),('Anonymous14', ANONYMOUS14),
    )
...
#IManagedObject Definition

#################################################################################

MSRPC_UUID_IMANAGEDOBJECT = uuidtup_to_bin(('C3FCC19E-A970-112-85-0009794','0.0'))


class GetSerializedBuffer(NDRCALL):
    opnum = 0
    structure = (

    )

class GetSerializedBufferResponse(NDRCALL):
    structure = (
		('pBSTR', BSTR),
    )
        

class GetObjectIdentity(NDRCALL):
    opnum = 1
    structure = (

    )

class GetObjectIdentityResponse(NDRCALL):
    structure = (
		('pBSTRGUID', BSTR),
		('AppDomainID', INT),
		('pCCW', CCW_PTR),
...
OPNUMS = {
0 : (GetSerializedBuffer,GetSerializedBufferResponse),
1 : (GetObjectIdentity,GetObjectIdentityResponse),
}

#################################################################################

#IRemoteDispatch Definition

#################################################################################

MSRPC_UUID_IREMOTEDISPATCH = uuidtup_to_bin(('6619a740-8154-43be-a186-0319578e02db','0.0'))


class RemoteDispatchAutoDone(NDRCALL):
    opnum = 0
    structure = (
		('s', BSTR),
    )

class RemoteDispatchAutoDoneResponse(NDRCALL):
    structure = (
		('pRetVal', BSTR),
    )
        

class RemoteDispatchNotAutoDone(NDRCALL):
    opnum = 1
    structure = (
		('s', BSTR),
    )

class RemoteDispatchNotAutoDoneResponse(NDRCALL):
    structure = (
		('pRetVal', BSTR),
    )
        
OPNUMS = {
0 : (RemoteDispatchAutoDone,RemoteDispatchAutoDoneResponse),
1 : (RemoteDispatchNotAutoDone,RemoteDispatchNotAutoDoneResponse),
}

#################################################################################

#IServicedComponentInfo Definition

#################################################################################

MSRPC_UUID_ISERVICEDCOMPONENTINFO = uuidtup_to_bin(('8165B19E-8D3A-4d0b-80C8-97DE310DB583','0.0'))


class GetComponentInfo(NDRCALL):
    opnum = 0
    structure = (
		('infoMask', INT),
    )

class GetComponentInfoResponse(NDRCALL):
    structure = (
		('infoMask', INT),
		('infoArray', SAFEARRAY ( BSTR )),
    )
        
OPNUMS = {
0 : (GetComponentInfo,GetComponentInfoResponse),
}
