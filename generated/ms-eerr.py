...
class U0(NDRUNION):
    union = {
        1: ('AnsiString',EEASTRING),2: ('UnicodeString',EEUSTRING),3: ('LVal',LONG),4: ('IVal',SHORT),5: ('PVal',__INT64),7: ('Blob',BINARYEEINFO),
    }
        

class EXTENDEDERRORPARAM(NDRSTRUCT):
    structure = (
        ('Type', EXTENDEDERRORPARAMTYPESINTERNAL),('u0', U0),
    )
...
class EECOMPUTERNAME(NDRSTRUCT):
    structure = (
        ('Type', EECOMPUTERNAMEPRESENT),('u0', U0),
    )


class NEXT(NDRSTRUCT):
    structure = (
        
    )


class EXTENDEDERRORINFO(NDRSTRUCT):
    structure = (
        ('NEXT', NEXT),('ComputerName', EECOMPUTERNAME),('ProcessID', UNSIGNED_LONG),('TimeStamp', __INT64),('GeneratingComponent', UNSIGNED_LONG),('Status', UNSIGNED_LONG),('DetectionLocation', UNSIGNED_SHORT),('Flags', UNSIGNED_SHORT),('nLen', SHORT),('Params', EXTENDEDERRORPARAM),
    )

EXTENDEDERRORINFOPTR = EXTENDEDERRORINFO
OPNUMS = {
}
