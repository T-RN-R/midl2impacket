...
class CONTAINERSTATISTICS(NDRSTRUCT):
    structure = (
        ('cCalls', DWORD),('cComponentInstances', DWORD),('cComponents', DWORD),('cCallsPerSecond', DWORD),
    )


class CONTAINERDATA(NDRSTRUCT):
    structure = (
        ('dwLegacyId', DWORD),('wszApplicationIdentifier', WCHAR),('dwProcessId', DWORD),('statistics', CONTAINERSTATISTICS),
    )


class COMPONENTDATA(NDRSTRUCT):
    structure = (
        ('clsid', GUID),('cTotalReferences', DWORD),('cBoundReferences', DWORD),('cPooledInstances', DWORD),('cInstancesInCall', DWORD),('dwResponseTime', DWORD),('cCallsCompleted', DWORD),('cCallsFailed', DWORD),
    )
...
#IGetTrackingData Definition

#################################################################################

MSRPC_UUID_IGETTRACKINGDATA = uuidtup_to_bin(('B60040E0-BCF3-111-861-0080729264','0.0'))
...
class GetContainerData(NDRCALL):
    opnum = 1
    structure = (

    )

class GetContainerDataResponse(NDRCALL):
    structure = (
		('nContainers', DWORD),
		('aContainerData', CONTAINERDATA),
    )
        

class GetComponentDataByContainer(NDRCALL):
    opnum = 2
    structure = (
		('idContainer', DWORD),
    )

class GetComponentDataByContainerResponse(NDRCALL):
    structure = (
		('nComponents', DWORD),
		('aComponentData', COMPONENTDATA),
    )
        

class GetComponentDataByContainerAndCLSID(NDRCALL):
    opnum = 3
    structure = (
		('idContainer', DWORD),
		('clsid', GUID),
...
class GetComponentDataByContainerAndCLSIDResponse(NDRCALL):
    structure = (
		('ppComponentData', COMPONENTDATA),
    )
...
1 : (GetContainerData,GetContainerDataResponse),
2 : (GetComponentDataByContainer,GetComponentDataByContainerResponse),
3 : (GetComponentDataByContainerAndCLSID,GetComponentDataByContainerAndCLSIDResponse),
4 : (Opnum7NotUsedOnWire,Opnum7NotUsedOnWireResponse),
}

#################################################################################

#IComTrackingInfoEvents Definition

#################################################################################

MSRPC_UUID_ICOMTRACKINGINFOEVENTS = uuidtup_to_bin(('4E6CDCC9-FB25-4FD5-9CC5-C9F4B6559CEC','0.0'))


class OnNewTrackingInfo(NDRCALL):
    opnum = 0
    structure = (
		('pToplevelCollection', IUNKNOWN),
    )

class OnNewTrackingInfoResponse(NDRCALL):
    structure = (

    )
        
OPNUMS = {
0 : (OnNewTrackingInfo,OnNewTrackingInfoResponse),
}

#################################################################################

#IProcessDump Definition

#################################################################################

MSRPC_UUID_IPROCESSDUMP = uuidtup_to_bin(('23C9DD26-2355-4FE2-84DE-F779A238ADBD','0.0'))


class IsSupported(NDRCALL):
    opnum = 0
    structure = (

    )

class IsSupportedResponse(NDRCALL):
    structure = (

    )
        

class DumpProcess(NDRCALL):
    opnum = 1
    structure = (
...
class DumpProcessResponse(NDRCALL):
    structure = (
		('pbstrDumpFile', BSTR),
    )
        
OPNUMS = {
0 : (IsSupported,IsSupportedResponse),
1 : (DumpProcess,DumpProcessResponse),
}
