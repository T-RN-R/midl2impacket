...
class SAFEARRAYUNION(NDRSTRUCT):
    structure = (
        ('sfType', UNSIGNED_LONG),('Anonymous27', ANONYMOUS27),
    )
...
class VSS_SNAPSHOT_PROP(NDRSTRUCT):
    structure = (
        ('m_SnapshotId', VSS_ID),('m_SnapshotSetId', VSS_ID),('m_lSnapshotsCount', LONG),('m_pwszSnapshotDeviceObject', VSS_PWSZ),('m_pwszOriginalVolumeName', VSS_PWSZ),('m_pwszOriginatingMachine', VSS_PWSZ),('m_pwszServiceMachine', VSS_PWSZ),('m_pwszExposedName', VSS_PWSZ),('m_pwszExposedPath', VSS_PWSZ),('m_ProviderId', VSS_ID),('m_lSnapshotAttributes', LONG),('m_tsCreationTimestamp', VSS_TIMESTAMP),('m_eStatus', VSS_SNAPSHOT_STATE),
    )


class VSS_PROVIDER_PROP(NDRSTRUCT):
    structure = (
        ('m_ProviderId', VSS_ID),('m_pwszProviderName', VSS_PWSZ),('m_eProviderType', VSS_PROVIDER_TYPE),('m_pwszProviderVersion', VSS_PWSZ),('m_ProviderVersionId', VSS_ID),('m_ClassId', CLSID),
    )


class VSS_OBJECT_UNION(NDRUNION):
    union = {
        VSS_OBJECT_SNAPSHOT: ('Snap',VSS_SNAPSHOT_PROP),VSS_OBJECT_PROVIDER: ('Prov',VSS_PROVIDER_PROP),
    }
        

class VSS_OBJECT_PROP(NDRSTRUCT):
    structure = (
...
class VSS_VOLUME_PROP(NDRSTRUCT):
    structure = (
        ('m_pwszVolumeName', VSS_PWSZ),('m_pwszVolumeDisplayName', VSS_PWSZ),
    )


class VSS_DIFF_VOLUME_PROP(NDRSTRUCT):
    structure = (
        ('m_pwszVolumeName', VSS_PWSZ),('m_pwszVolumeDisplayName', VSS_PWSZ),('m_llVolumeFreeSpace', LONGLONG),('m_llVolumeTotalSpace', LONGLONG),
    )


class VSS_DIFF_AREA_PROP(NDRSTRUCT):
    structure = (
        ('m_pwszVolumeName', VSS_PWSZ),('m_pwszDiffAreaVolumeName', VSS_PWSZ),('m_llMaximumDiffSpace', LONGLONG),('m_llAllocatedDiffSpace', LONGLONG),('m_llUsedDiffSpace', LONGLONG),
    )


class VSS_MGMT_OBJECT_UNION(NDRUNION):
    union = {
        VSS_MGMT_OBJECT_VOLUME: ('Vol',VSS_VOLUME_PROP),VSS_MGMT_OBJECT_DIFF_VOLUME: ('DiffVol',VSS_DIFF_VOLUME_PROP),VSS_MGMT_OBJECT_DIFF_AREA: ('DiffArea',VSS_DIFF_AREA_PROP),
    }
        

class VSS_MGMT_OBJECT_PROP(NDRSTRUCT):
    structure = (
        ('Type', VSS_MGMT_OBJECT_TYPE),('Obj', VSS_MGMT_OBJECT_UNION),
    )
...
MSRPC_UUID_IVSSSNAPSHOTMGMT = uuidtup_to_bin(('FA7DF749-667-4986-A27F-E2F04AE53772','0.0'))


class GetProviderMgmtInterface(NDRCALL):
    opnum = 0
    structure = (
		('ProviderId', VSS_ID),
		('InterfaceId', REFIID),
...
class GetProviderMgmtInterfaceResponse(NDRCALL):
    structure = (
		('ppItf', IUNKNOWN),
    )
        

class QueryVolumesSupportedForSnapshots(NDRCALL):
    opnum = 1
    structure = (
		('ProviderId', VSS_ID),
		('lContext', LONG),
...
class QueryVolumesSupportedForSnapshotsResponse(NDRCALL):
    structure = (
		('ppEnum', IVSSENUMMGMTOBJECT),
    )
        

class QuerySnapshotsByVolume(NDRCALL):
    opnum = 2
    structure = (
...
class QuerySnapshotsByVolumeResponse(NDRCALL):
    structure = (
		('ppEnum', IVSSENUMOBJECT),
    )
        
OPNUMS = {
0 : (GetProviderMgmtInterface,GetProviderMgmtInterfaceResponse),
1 : (QueryVolumesSupportedForSnapshots,QueryVolumesSupportedForSnapshotsResponse),
2 : (QuerySnapshotsByVolume,QuerySnapshotsByVolumeResponse),
}

#################################################################################

#IVssDifferentialSoftwareSnapshotMgmt Definition

#################################################################################

MSRPC_UUID_IVSSDIFFERENTIALSOFTWARESNAPSHOTMGMT = uuidtup_to_bin(('214A0F28-B737-4026-B847-4F9E37D79529','0.0'))


class AddDiffArea(NDRCALL):
    opnum = 0
    structure = (
		('pwszVolumeName', VSS_PWSZ),
		('pwszDiffAreaVolumeName', VSS_PWSZ),
		('llMaximumDiffSpace', LONGLONG),
    )

class AddDiffAreaResponse(NDRCALL):
    structure = (

    )
        

class ChangeDiffAreaMaximumSize(NDRCALL):
    opnum = 1
    structure = (
		('pwszVolumeName', VSS_PWSZ),
		('pwszDiffAreaVolumeName', VSS_PWSZ),
...
class ChangeDiffAreaMaximumSizeResponse(NDRCALL):
    structure = (

    )
        

class QueryVolumesSupportedForDiffAreas(NDRCALL):
    opnum = 2
    structure = (
		('pwszOriginalVolumeName', VSS_PWSZ),
    )

class QueryVolumesSupportedForDiffAreasResponse(NDRCALL):
    structure = (
		('ppEnum', IVSSENUMMGMTOBJECT),
    )
        

class QueryDiffAreasForVolume(NDRCALL):
    opnum = 3
    structure = (
		('pwszVolumeName', VSS_PWSZ),
    )

class QueryDiffAreasForVolumeResponse(NDRCALL):
    structure = (
		('ppEnum', IVSSENUMMGMTOBJECT),
    )
        

class QueryDiffAreasOnVolume(NDRCALL):
    opnum = 4
    structure = (
		('pwszVolumeName', VSS_PWSZ),
    )

class QueryDiffAreasOnVolumeResponse(NDRCALL):
    structure = (
		('ppEnum', IVSSENUMMGMTOBJECT),
    )
        

class Opnum08NotUsedOnWire(NDRCALL):
    opnum = 5
    structure = (

    )

class Opnum08NotUsedOnWireResponse(NDRCALL):
    structure = (

    )
        
OPNUMS = {
0 : (AddDiffArea,AddDiffAreaResponse),
1 : (ChangeDiffAreaMaximumSize,ChangeDiffAreaMaximumSizeResponse),
2 : (QueryVolumesSupportedForDiffAreas,QueryVolumesSupportedForDiffAreasResponse),
3 : (QueryDiffAreasForVolume,QueryDiffAreasForVolumeResponse),
4 : (QueryDiffAreasOnVolume,QueryDiffAreasOnVolumeResponse),
5 : (Opnum08NotUsedOnWire,Opnum08NotUsedOnWireResponse),
}

#################################################################################

#IVssEnumObject Definition

#################################################################################

MSRPC_UUID_IVSSENUMOBJECT = uuidtup_to_bin(('AE1C7110-260-113-839-00047283','0.0'))
...
#IVssEnumMgmtObject Definition

#################################################################################

MSRPC_UUID_IVSSENUMMGMTOBJECT = uuidtup_to_bin(('01954E6B-9254-4e6e-808C-C9E05D007696','0.0'))
