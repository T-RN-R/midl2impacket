...
class DNS_RPC_TRUST_POINT(NDRSTRUCT):
	...
	structure = (
			...
			(
			'i64NextActiveRefreshTime',
			__INT64
			),
			(
			'i64LastSuccessfulActiveRefreshTime',
			__INT64
			),
			(
			'dwLastActiveRefreshResult',
			DWORD
			),
			(
			'dwReserved',
...
class DNS_RPC_SKD_STATE_EX(NDRSTRUCT):
	...
	structure = (
			...
			(
			'dwRevokedOrSwappedDnskeysLength',
			DWORD
			),
			(
			'pRevokedOrSwappedDnskeysBuffer',
			PBYTE
			),
			(
			'dwFinalDnskeysLength',
			DWORD
			),
			(
			'pFinalDnskeys',
...
class DNS_RPC_ZONE_DNSSEC_SETTINGS(NDRSTRUCT):
	...
	structure = (
			...
			(
			'dwMaintainTrustAnchor',
			DWORD
			),
			(
			'fParentHasSecureDelegation',
			DWORD
			),
			(
			'dwDSRecordAlgorithms',
			DWORD
			),
			(
			'fRFC5011KeyRollovers',
			DWORD
			),
			(
			'bNSEC3HashAlgorithm',
			...
			(
			'bNSEC3RandomSaltLength',
			BYTE
			),
			(
			'wNSEC3IterationCount',
			WORD
			),
			(
			'pwszNSEC3UserSalt',
			...
			(
			'dwSignatureInceptionOffset',
			DWORD
			),
			(
			'dwSecureDelegationPollingPeriod',
			DWORD
			),
			(
			'dwPropagationTime',
...
class PTR_DNS_RPC_UNICODE_STRING_LIST(NDRPOINTER):
	referent = (
			(
			'Data',
			DATA_DNS_RPC_UNICODE_STRING_LIST
			)
		)
...
class DNS_RPC_UNICODE_STRING_LIST(NDRSTRUCT):
	...
	structure = (
			...
			(
			'pwszStrings',
			PTR_DNS_RPC_UNICODE_STRING_LIST
			)
		)


DNS_RPC_CRITERIA_COMPARATOR = DWORD__ENUM
Equals = 1
DNS_RPC_POLICY_CONDITION = DWORD__ENUM
DNS_AND = 0
DNS_RPC_POLICY_LEVEL = DWORD__ENUM
DnsPolicyServerLevel = 0
DnsPolicyZoneLevel = 1
DNS_RPC_POLICY_ACTION_TYPE = DWORD__ENUM
DnsPolicyDeny = 0
DnsPolicyAllow = 1
DnsPolicyIgnore = 2
DNS_RPC_POLICY_TYPE = DWORD__ENUM
DnsPolicyQueryProcessing = 0
DnsPolicyZoneTransfer = 1
DnsPolicyDynamicUpdate = 2
DnsPolicyRecursion = 3
DNS_RPC_CRITERIA_ENUM = DWORD__ENUM
...
class PDNS_RPC_VIRTUALIZATION_INSTANCE(NDRPOINTER):
	referent = (
			(
			'Data',
			DNS_RPC_VIRTUALIZATION_INSTANCE
			)
		)
...
class PDNS_RPC_VIRTUALIZATION_INSTANCE_INFO(NDRPOINTER):
	referent = (
			(
			'Data',
			DNS_RPC_VIRTUALIZATION_INSTANCE_INFO
			)
		)
...
class PDNS_RPC_ENUM_VIRTUALIZATION_INSTANCE_LIST(NDRPOINTER):
	referent = (
			(
			'Data',
			DNS_RPC_ENUM_VIRTUALIZATION_INSTANCE_LIST
			)
		)


DNS_RPC_TYPEID = DWORD__ENUM
DNSSRV_TYPEID_ANY = -1)
...
DNS_ZONE_STATS_TYPE = DWORD__ENUM
...
ImportOpResult = DWORD__ENUM
IMPORT_STATUS_NOOP = 0
IMPORT_STATUS_SIGNING_READY = 1
IMPORT_STATUS_UNSIGNING_READY = 2
#################################################################################
...
OPNUMS = {0 : (
	R_DnssrvOperation,
	R_DnssrvOperationResponse
	),1 : (
	R_DnssrvQuery,
	R_DnssrvQueryResponse
	),2 : (
	R_DnssrvComplexOperation,
	R_DnssrvComplexOperationResponse
	),3 : (
	R_DnssrvEnumRecords,
	R_DnssrvEnumRecordsResponse
	),4 : (
	R_DnssrvUpdateRecord,
	R_DnssrvUpdateRecordResponse
	),5 : (
	R_DnssrvOperation2,
	R_DnssrvOperation2Response
	),6 : (
	R_DnssrvQuery2,
	R_DnssrvQuery2Response
	),7 : (
	R_DnssrvComplexOperation2,
	R_DnssrvComplexOperation2Response
	),8 : (
	R_DnssrvEnumRecords2,
	R_DnssrvEnumRecords2Response
	),9 : (
	R_DnssrvUpdateRecord2,
	R_DnssrvUpdateRecord2Response
	),10 : (
	R_DnssrvUpdateRecord3,
	R_DnssrvUpdateRecord3Response
	),11 : (
	R_DnssrvEnumRecords3,
	R_DnssrvEnumRecords3Response
	),12 : (
	R_DnssrvOperation3,
	R_DnssrvOperation3Response
	),13 : (
	R_DnssrvQuery3,
	R_DnssrvQuery3Response
	),14 : (
	R_DnssrvComplexOperation3,
	R_DnssrvComplexOperation3Response
	),15 : (
	R_DnssrvOperation4,
	R_DnssrvOperation4Response
	),16 : (
	R_DnssrvQuery4,
	R_DnssrvQuery4Response
	),17 : (
	R_DnssrvUpdateRecord4,
	R_DnssrvUpdateRecord4Response
	),18 : (
	R_DnssrvEnumRecords4,
	R_DnssrvEnumRecords4Response
	)}
